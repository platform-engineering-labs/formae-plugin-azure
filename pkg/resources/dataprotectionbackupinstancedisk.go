...
const ResourceTypeDataProtectionBackupInstanceDisk = "AZURE::DataProtection::BackupInstanceDisk"

// datasourceTypeManagedDisk is both the `datasourceType` ARM matches against the
// policy's own datasourceTypes and the ARM type of the disk itself — unlike blob,
// where the datasource is the blob service but the ID points at the account.
const datasourceTypeManagedDisk = "Microsoft.Compute/disks"

// dpDiskBackupInstancesAPI is the armdataprotection surface used here. Create and
...
type dpDiskBackupInstancesAPI interface {
...
func init() {
	registry.Register(ResourceTypeDataProtectionBackupInstanceDisk, func(c *client.Client, cfg *config.Config) prov.Provisioner {
...
// DataProtectionBackupInstanceDisk is the provisioner for a managed-disk backup
// instance (`Microsoft.DataProtection/backupVaults/backupInstances`, datasource
// `Microsoft.Compute/disks`).
type DataProtectionBackupInstanceDisk struct {
	api      dpDiskBackupInstancesAPI
	pipeline runtime.Pipeline
	config   *config.Config
}

// dpDiskInstanceProps mirrors
// schema/pkl/dataprotection/dataprotectionbackupinstancedisk.pkl.
type dpDiskInstanceProps struct {
	...
	SnapshotResourceGroupID    string `json:"snapshotResourceGroupId"`
	UserAssignedIdentityArmURL string `json:"userAssignedIdentityArmUrl"`
}

func (d *DataProtectionBackupInstanceDisk) buildInstance(props dpDiskInstanceProps) (*armdataprotection.BackupInstance, error) {
	...
	// Disk backup writes incremental snapshots into a resource group the caller
	// nominates, and bills them there. ARM carries it as a DataStoreParameters
	// entry on the operational tier rather than as a top-level field.
	if props.SnapshotResourceGroupID != "" {
		policyInfo.PolicyParameters = &armdataprotection.PolicyParameters{
			DataStoreParametersList: []armdataprotection.DataStoreParametersClassification{
				&armdataprotection.AzureOperationalStoreParameters{
					...
					ResourceGroupID: to.Ptr(props.SnapshotResourceGroupID),
				},
			},
		}
	}
	...
	instance := &armdataprotection.BackupInstance{
		...
		DataSourceInfo: &armdataprotection.Datasource{
			...
			ResourceType:     to.Ptr(datasourceTypeManagedDisk),
			DatasourceType:   to.Ptr(datasourceTypeManagedDisk),
...
func (d *DataProtectionBackupInstanceDisk) buildPropertiesFromResult(res *armdataprotection.BackupInstanceResource, rgName, vaultName string) map[string]any {
	...
	if p := res.Properties; p != nil {
		...
		if pi := p.PolicyInfo; pi != nil {
			...
			if pp := pi.PolicyParameters; pp != nil {
				for _, raw := range pp.DataStoreParametersList {
					opStore, ok := raw.(*armdataprotection.AzureOperationalStoreParameters)
					if !ok {
						continue
					}
					if opStore.ResourceGroupID != nil {
						props["snapshotResourceGroupId"] = *opStore.ResourceGroupID
					}
				}
			}
		}
...
func (d *DataProtectionBackupInstanceDisk) Create(ctx context.Context, request *resource.CreateRequest) (*resource.CreateResult, error) {
	var props dpDiskInstanceProps
...
func (d *DataProtectionBackupInstanceDisk) Read(ctx context.Context, request *resource.ReadRequest) (*resource.ReadResult, error) {
	...
	return &resource.ReadResult{
		ResourceType: ResourceTypeDataProtectionBackupInstanceDisk,
...
func (d *DataProtectionBackupInstanceDisk) Update(ctx context.Context, request *resource.UpdateRequest) (*resource.UpdateResult, error) {
	rgName, vaultName, name, err := dpBackupInstanceIDParts(request.NativeID)
	if err != nil {
		return nil, err
	}
...
func (d *DataProtectionBackupInstanceDisk) Delete(ctx context.Context, request *resource.DeleteRequest) (*resource.DeleteResult, error) {
	rgName, vaultName, name, err := dpBackupInstanceIDParts(request.NativeID)
	if err != nil {
		return nil, err
	}
...
func (d *DataProtectionBackupInstanceDisk) Status(ctx context.Context, request *resource.StatusRequest) (*resource.StatusResult, error) {
...
func (d *DataProtectionBackupInstanceDisk) completeFromInstance(res *armdataprotection.BackupInstanceResource) (string, json.RawMessage, error) {
	nativeID := ""
	rgName, vaultName := "", ""
	if res.ID != nil {
		nativeID = *res.ID
...
// to disks; otherwise this type would also claim the blob-storage instances.
func (d *DataProtectionBackupInstanceDisk) List(ctx context.Context, request *resource.ListRequest) (*resource.ListResult, error) {
	...
	for pager.More() {
		...
		for _, item := range page.Value {
			...
			if !dpInstanceHasDatasource(item, datasourceTypeManagedDisk) {
				continue
			}
